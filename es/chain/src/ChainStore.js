...
var ChainStore = /*#__PURE__*/function () {
  ...
  _proto.getLiquidityPoolsByAssets = function getLiquidityPoolsByAssets(assetType, assetA, assetB, limit, startId) {
    ...
    return Apis.instance().db_api().exec(methodName, params).then(function (result) {
      if (result.length > 0) {
        var tmp = [];
        result.forEach(function (pool) {
          pool.asset_a = _this16.getAsset(pool.asset_a);
          pool.asset_b = _this16.getAsset(pool.asset_b);
          pool.share_asset = _this16.getAsset(pool.share_asset);
          pool.dynamic_share_asset = _this16.getObject(pool.share_asset.get('dynamic_asset_data_id'));
          tmp.push(pool);
        });
  ...
  _proto.getLiquidityPoolsByShareAsset = function getLiquidityPoolsByShareAsset(assets, subscribe) {
    ...
    return Apis.instance().db_api().exec('get_liquidity_pools_by_share_asset', [assets, subscribe]).then(function (result) {
      if (result.length > 0) {
        var tmp = [];
        result.forEach(function (pool) {
          pool.asset_a = _this17.getAsset(pool.asset_a);
          pool.asset_b = _this17.getAsset(pool.asset_b);
          pool.share_asset = _this17.getAsset(pool.share_asset);
          pool.dynamic_share_asset = _this17.getObject(pool.share_asset.get('dynamic_asset_data_id'));
          tmp.push(pool);
        });
        return Immutable.fromJS(tmp);
      }
